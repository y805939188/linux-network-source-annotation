...
/**
 * 3.网卡发包. dev_hard_start_xmit 方法是当网卡驱动比较简单的时候, 比如是个回环设备之类的情况会被调用
 *   作用是用来发包
 * 	 内部调用 xmit_one 方法, 该方法中最终调用 ndo_start_xmit 方法
 * 	 而 ndo_start_xmit 方法是在最开始注册驱动之前就创建好的那个 net_device_ops 上赋值的
 * 
 * 8.网卡发包. dev_hard_start_xmit 如果是经过比较复杂的 dequeue 算法拿到的 skb 包, 会经过这个函数发送包
 */
...
/**
 * 5.网卡发包. __dev_xmit_skb 方法内部调用 __qdisc_run 方法dua挺 qdisc_restart
 * 
 * 	 qdisc_restart 方法中调用 dequeue_skb(), 这个 dequeue_skb 就会从 skb 链表中取出一个包
 * 	 然后发到网络驱动上
 */
...
/**
 * 2.网卡发包. __dev_queue_xmit 函数负责接收从协议栈下来的 skb, 然后给发到网卡上发送出去
 * 	 如果网络驱动设备支持优先级队列的话, 就优先使用 trace_net_dev_queue 去查找优先队列的算法
 *   用算法算出发送顺序
 * 
...
/**
 * 4.网卡收包. net_rx_action 会触发软中断
 * 	 对于网卡设备来讲, 就是触发 e1000_clean 方法
 * 	 work = n->poll(n, weight); 这行代码来调用
 */
...
/**
 * 7.驱动初始化. register_netdev 方法调用 register_netdevice 以注册设备
 *   然后将 net_device 结构插入到全局列表和 hash 表
 * 	 并对一些参数进行初始化, 产生一个广播, 通知其他内核组件有关此次的注册
 */
