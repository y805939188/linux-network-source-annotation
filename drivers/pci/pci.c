...
/**
 * 3.驱动初始化. pci_enable_device_flags函数中主要会调用 do_pci_enable_device()
 * 		do_pci_enable_device 方法中会使用 pci_set_power_state 方法, 该方法对设备的状态做一些设置, 比如设置管理设备的电源相关的寄存器等
 * 	 	do_pci_enable_device 方法中还会用 pcibios_enable_device 方法, 该方法用来设置 i/o 和内存映射等
 * 		do_pci_enable_device 方法中同时用 pci_read_config_byte 方法, 该方法用来读取设备被分配到的中断号
 * 				网卡有配置空间和寄存器空间
 * 				CPU 不能直接访问配置空间, 需要借助 BIOS 的能力
...
/**
 * 2.驱动初始化. pci_enable_device 方法中
 * 	 主要用来分配 I/O 和 memory region, 以及分配 IRQ, 也就是中断向量号
 * 	
 *   pci_enable_device_flags 方法中会调用 do_pci_enable_device()
 */
