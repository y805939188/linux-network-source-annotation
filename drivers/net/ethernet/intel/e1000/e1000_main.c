...
/**
 * 2.驱动注册. e1000_init_module
 * 		该函数用来向 PCI 子系统注册一个 pci 专用的结构体到 pci 的链表中
 * 		当内核去扫描 pci 上的设备时候会去找 pci 链表中对应编号的驱动程序
 * 		然后自动执行驱动程序的 probe 函数(对于 e1000 网卡来讲就是 e1000_probe)
 * 
 * 		注册过程, 主要是通过 pci_register_driver 函数注册 e1000_driver 结构体到 pci 链表中
 */
...
/**
 * 1.驱动注册. module_init(e1000_init_module);
 * 		操作系统内核会在特定的时机执行 module_init 注册过的函数
 * 		e1000_init_module 就是网卡驱动加载的第一个函数
 */
...
/**
 * 2.网卡收包. e1000_request_irq 方法中使用 request_irq 去申请一个中断
 */
...
/**
 *  0.网卡收包. 网卡 e1000
 * 	e1000 是一个网卡, 每个网卡都有自己的 DMA
 * 	DMA 就是硬件和操作系统内核共享的一段缓冲区
 * 	内核往字节的内存缓冲区中发送数据, 相当于往这个 DMA 中发
 * 	DMA 和操作系统的内核空间有映射关系, 操作系统就可以在 DMA 中读取数据
 * 	DMA 可能是个无锁环装缓冲区的数据结构
 * 	网卡有很多选项 ↓
 ...
 *
 * 
 * 	DMA 其实是个硬件, 全称叫 direct memory access 直接内存访问
 * 	DMA 是硬件上的一个控制器, 因为 cpu 经常要去做一些拷贝的事情，但是拷贝的事情可能比较耗时间，于是在硬件上加了个东西专门用来拷贝
 *  给 DMA 输入一个源地址，目的地址，大小，这仨有了之后 DMA 就主动进行拷贝，cpu 该干嘛干嘛。等拷贝完之后，DMA 发出一个中断告诉 cpu 拷贝完成
 *  可以提高 cpu 的效率，经常用在 io 外设的数据往内存(sdram)中拷贝的场景
 * 
 * 	上头有很多寄存器, 其中有用来设置 源地址/目的地址/大小 的寄存器,
 *  可以通过操作这些寄存器来做到将数据从一个地方映射到另外一个地方的功能
 * 
 * 
 *
 * 
...
static const struct net_device_ops e1000_netdev_ops = { 
	...
	/**
	 * 4.网卡发包. ndo_start_xmit 方法就是在这里被赋值的
	 * 
	 * 	 e1000_xmit_frame 方法中会触发一个软中断, 以用来发包到网卡
	 * 	 发完之后会做一些缓存的清理
	 */
...
/**
 * 1. 驱动初始化 e1000_probe 方法
 * 		在函数 e1000_probe() 中首先调用函数 pci_enable_device() 启用设备
 * 		然后声明了 DMA 空间, DMA 空间就是硬件的内存和内核内存的映射
 *    接着调用函数 alloc_etherdev() 生成结构体 net_device 该结构体就表示了网卡设备
 * 		之后对 net_device 的参数进行了初始化后, 比如让它持有 netdev_ops(包含e1000_open) 之类的属性
 * 		然后调用 register_netdev() 注册该设备
 * 
 * 		再然后使用 netif_napi_add, 给设备进行 napi 的注册, 注册一个叫 e1000_clean 的函数
 * 		就是说每次轮询这段缓冲区时候, 如果有东西, 就读出来调用 e1000_clean 函数
 * 
 */
static int e1000_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (need_ioport) {
		 ...
		 * 用来启动一个 pci 设备
		 * PCI 是一种硬件链接的规范或者说接口
		 * 硬件按照 PCI 规范实现自己的什么引脚啊, 线路之类的
		 * 然后插到其他硬件上, 其他硬件的内核就可以扫描它的引脚之类的东西从而用软件操作
		 * 
		 * 比如网卡, 接在 PCI 总线上, 然后内核扫描所有连在总线上的设备, 并调用每个设备的 probe 函数(也可能扫描所有的驱动程序, 根据驱动程序找设备)
		 */
	...
	/**
	 * 4.驱动初始化. e1000_netdev_ops 是提前申请好的结构体, 里头包括 e1000_open 之类的方法
	 */
	...
	/**
	 * 5.驱动初始化. 添加一个 napi, 用来当网卡触发了中断时, 不停的轮询触发 e1000_clean 回调
	 * 
	 * 网卡是一个 PCI 设备, 每个 PCI 有独立的设备号, 总线编号, 功能编号, 驱动程序也要有个号来标识要对哪个设备生效
	 * BIOS 启动时扫描 PCI 总线上的所有设备, 并分配给他们 mac 地址
	 */
	...
	/**
	 * 6.驱动初始化. e1000_sw_init 函数中负责使用 ioremap 方法将网卡的硬件地址与内存虚拟地址进行映射
	 */
 ...
 * 
 * 	 然后再 e1000_open 方法中会使用 e1000_request_irq 方法, 该方法会注册中断函数 e1000_intr
 */
int e1000_open(struct net_device *netdev)
{
	struct e1000_adapter *adapter = netdev_priv(netdev);
...
/**
 * 3.网卡收包. e1000_intr 方法是当网卡收到消息之后会立刻触发的中断上半部分
 * 	 也就是硬中断.
 * 
 *   它调用的实际上是 __napi_schedule
 * 	 __napi_schedule 最终会调用 __raise_softirq_irqoff, 这是触发一个软中断的意思
 * 
 * 	 这个软中断最终执行的函数是 net_rx_action
 */
static irqreturn_t e1000_intr(int irq, void *data)
{
...
/**
 * 5.网卡收包. e1000_clean 方法就是当网卡收到数据包之后会先触发一个硬中断
 * 	 然后这个硬中断中会触发一个软中断的 napi, 这个 napi 中使用一个 poll 函数不停轮询
 * 	 触发的就是这个 e1000_clean 方法
 * 
 * 	 这里头主要调用的是 e1000_clean_tx_irq()
 */
static int e1000_clean(struct napi_struct *napi, int budget)
{
...
/**
 * 6.网卡收包. e1000_clean_tx_irq(还是走 e1000_clean_rx_irq ???) 方法会从硬件或者说 DMA 中把包给拿到手
 * 	 里头有个 while 循环, 如果 !cleaned 就一直读取
 *   for 循环主要做的事情就是把局部的 buffer_info -> skb 映射到 dma(也就是这个 tx_ring)
 *   然后当这个 dma 收到了网卡接收到的包时候就相当于自动映射到了 buffer_info
 * 	 之后清理 dma 营社区, 并且申请同等数量的 skb, 把这些 skb 再映射到 dma 中
 * 
 * 	 当收干净之后, 走到 precess_skb 阶段的逻辑
 */
static bool e1000_clean_tx_irq(struct e1000_adapter *adapter,
			       struct e1000_tx_ring *tx_ring)
{
	...
	while ((eop_desc->upper.data & cpu_to_le32(E1000_TXD_STAT_DD)) &&
		...
		for ( ; !cleaned; count++) {
			...
			/**
			 * 清理中间信息.
			 * 调用 e1000_clean_tx_irq 释放已经发送完成的skb内存, 解除skb的dma映射
			 */
			e1000_unmap_and_free_tx_resource(adapter, buffer_info);
...
/**
 * 7.网卡收包. process_skb 阶段主要执行 e1000_receive_skb 方法
 *   e1000_receive_skb 主要执行 napi_gro_receive()
